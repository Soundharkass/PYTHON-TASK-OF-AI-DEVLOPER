...
connector = p.connect(
    ...
    database="ExpenseDB"     # Make sure this DB exists
)
...
def add_another_expense():
	...
	if not date.get() or not payee.get() or not desc.get() or not amnt.get() or not MoP.get():
		...
		cursor.execute(
    'INSERT INTO ExpenseTracker (Date, Payee, Description, Amount, ModeOfPayment) VALUES (%s, %s, %s, %s, %s)',
...
def edit_expense():
	...
	def edit_existing_expense():
		...
		cursor.execute('UPDATE ExpenseTracker SET Date=%s, Payee=%s, Description=%s, Amount=%s, ModeOfPayment=%s WHERE ID=%s',
...
dataentery_frame_bg = 'lightpink'
buttons_frame_bg = 'beige'
hlb_btn_bg = 'lavender'
